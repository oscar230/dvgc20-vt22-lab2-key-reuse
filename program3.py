...
class Cipher:
    ...
    def to_plaintext(self, key: str) -> str:
        ...
        for i in range(0, len(key), 2):
            curr_char: str = self.cipher[i:i+1]
            if curr_char == common.PLACEHOLDER_CHAR:
                # Current character is a placeholder character
                # Should be represented by another character
                plaintext += common.PLACEHOLDER_CHAR_DISPLAY
            elif common.is_readable(curr_char):
                # Current character is readable
                # Should be added to key
                plaintext += common.xor_strings(curr_char, key[i+1])
            else:
...
def build_key(key_parts: list[KeyPart], key_len: int) -> str:
    key: str = "" # Hex
    for i in range(0, key_len, 2): # Skip every other becouse of hex being represenated as a string
        if len(key) < i:
...
if __name__ == "__main__":
    ...
    for word in words:
        ...
        for curr_pos in range(0, len(cipher_x.cipher) - len(word.word) + 1, 2):
            ...
            pick_options.append(f'{curr_pos}\t{common.try_hex_to_string(plaintext)} ({plaintext})')
        quit()
